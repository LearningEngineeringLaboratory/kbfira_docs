  ...
  handleRefresh() {
    ...
    this.session.getAll().then(sessions => { // console.log(sessions)
      ...
      Promise.all(promises).then(maps => {
        ...
        RecomposeApp.parseKitMapOptions(kitMap)
        if (kitMap && !learnerMap) RecomposeApp.resetMapToKit(kitMap, this.canvas)
        if (kitMap) {
          try {
            if (stateData && stateData.logger) {
              ...
              if (RecomposeApp.loggerListener)
                this.canvas.off("event", RecomposeApp.loggerListener);
              RecomposeApp.loggerListener = RecomposeApp.inst.logger.onCanvasEvent.bind(RecomposeApp.inst.logger);
              this.canvas.on("event", RecomposeApp.loggerListener);
            }
      ...
      this.canvas.on('event', RecomposeApp.onCanvasEvent)
